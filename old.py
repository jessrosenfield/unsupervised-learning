...
def kbest(tx, ty, rx, ry):
    print "kbest"
    for i in range(len(tx[0] - 2)):
        k = i + 1
        compressor = best(chi2, k=k)
        compressor.fit(tx, y=ty)
        newtx = compressor.transform(tx)
        newrx = compressor.transform(rx)
        em(newtx, ty, newrx, ry, add="wKBtr"+str(k))
        km(newtx, ty, newrx, ry, add="wKBtr"+str(k))
        nn(newtx, ty, newrx, ry, add="wKBtr"+str(k))
    print "kbest done"


def em(tx, ty, rx, ry, add="", times=12):
    print "em" + add
    errs = []
    ...
    for i in range(2,times):
        ...
        # a 1 or a 0
        mapper = {x: round(sum(ty[v] for v in clusters[x])/float(len(clusters[x]))) if clusters[x] else 0 for x in range(i)}
        ...
        sqrd_err = [(processed[n] - test[n])**2 for n in range(len(processed))]
        errs.append(sum(sqrd_err) / float(len(ry)))
